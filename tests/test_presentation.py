...
def test_operational_fact_rows_and_evidence_are_identifier_free():
    ...
    assert rows == [{
        "What Happened": "Physical contact",
    ...
    assert all(term not in str(rows) for term in ("Direction", "Assertion", "Resolution", "Temporal Scope"))
...
def test_policy_copy_and_semantic_summary_use_true_north_outcomes():
    ...
    assert "workplace violence criteria" in semantic_summary(result.validation_result, result.policy_decision)
...
def test_salesforce_rows_preserve_payload_order_and_readable_values():
    ...
    assert any(row["Field"] == "Outcome" for row in rows)
    assert any(row["Field"] == "Who or What Was Involved" for row in rows)
