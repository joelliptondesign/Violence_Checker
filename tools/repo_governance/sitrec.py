...
def render_sitrec(root: Path, operational_date: str, title: str = DEFAULT_TITLE) -> str:
    ...
    sections = (
        ...
        (
            ...
            f"The app accepts fixtures or manual narratives, shows lexical and proposition results, applies total deterministic policy, and builds a policy-gated illustrative Salesforce preview. Offline evaluation validates {facts.case_count} successor cases and supports explicit case selection, run creation, separately authorized baseline acceptance, within-family regression comparison, and evidence-only reports. Permanent Gate A–F tests and composed repository baseline-readiness governance protect the successor authority.",
        ),
        (
            "I. KNOWN LIMITATIONS",
