...
LIVE_EVALUATION_RUN_PATH = Path("evaluation/runs/true-north-live-evaluation-003.json")
LIVE_EVALUATION_COMPARISON_PATH = Path("evaluation/reports/true-north-live-comparison-003.json")
LIVE_EVALUATION_REPORT_PATH = Path("evaluation/reports/true-north-live-evaluation-003.md")
LIVE_FIXTURE_EVALUATION_PATH = Path("evaluation/reports/true-north-live-fixtures-003.json")
...
GROUNDING_ANCHORS = (
    ...
    (LIVE_FIXTURE_EVALUATION_PATH.as_posix(), "Observed eight-fixture live-provider evidence; not ground truth or an accepted baseline."),
...
class SitrecFacts:
    ...
    live_evaluation_acceptable_variations: int
    live_evaluation_availability: str
    live_fixture_provider_failures: int
...
def collect_sitrec_facts(root: Path, operational_date: str) -> SitrecFacts:
    ...
    if not isinstance(live_summary, dict) or live_comparison.get("execution", {}).get("attempted_corpus_cases") != len(cases):
        raise ValueError("live evaluation comparison does not cover the active corpus")
    readiness = live_comparison.get("semantic_readiness_conclusion")
    if readiness not in {
    ...
    return SitrecFacts(
        ...
        live_evaluation_passes=int(live_summary.get("exact_corpus_passes", 0)),
        live_evaluation_validation_failures=int(live_summary.get("corpus_status_counts", {}).get("VALIDATION_FAILURE", 0)),
        live_evaluation_semantic_mismatches=int(live_summary.get("corpus_status_counts", {}).get("SEMANTIC_MISMATCH", 0)),
        live_evaluation_provider_timeouts=int(live_summary.get("corpus_status_counts", {}).get("PROVIDER_TIMEOUT", 0)),
        live_evaluation_acceptable_variations=int(live_summary.get("acceptable_variations", 0)),
        live_evaluation_availability=str(live_comparison.get("provider_availability_conclusion", "missing")),
        live_fixture_provider_failures=int(live_summary.get("fixture_status_counts", {}).get("PROVIDER_FAILURE", 0)),
...
def render_sitrec(root: Path, operational_date: str, title: str = DEFAULT_TITLE) -> str:
    ...
    sections = (
        ...
        (
            "B. CURRENT STATE",
            f"Current repository facts: `TrueNorthSemanticEnvelope` is the sole current semantic authority. The True North runtime is active across extraction, repository adaptation, schema and domain validation, deterministic derivation and policy, application orchestration, comparison, illustrative Salesforce projection, bounded Operator Communication, presentation, and Streamlit. The repository-authored `{facts.corpus_identity}` contains {facts.case_count} synthetic operational cases under evaluation schema `{facts.corpus_schema_version}`, explicit expectations for all eight demonstration fixtures, and adversarial evidence-integrity coverage. The first 24-case live-provider evaluation is complete and remains immutable. The second live re-evaluation is also complete and remains immutable. The prior state that corrections are implemented but have not been live re-evaluated is superseded by these observed runs. The post-refactor live evaluation recorded {facts.live_evaluation_passes} exact passes, {facts.live_evaluation_acceptable_variations} acceptable variations, {facts.live_evaluation_validation_failures} evidence-integrity validation failures, {facts.live_evaluation_semantic_mismatches} semantic mismatches, and {facts.live_evaluation_provider_timeouts} provider timeouts; its semantic assessment is `{facts.live_evaluation_readiness}` and its independent availability assessment is `{facts.live_evaluation_availability}`. All eight fixtures were also attempted live, with {facts.live_fixture_provider_failures} provider request failures. Historical evaluation artifacts remain immutable, readable through their creation-time family, and non-authoritative. No True North baseline has been accepted; deployment and hosted acceptance remain pending.",
        ),
        (
            "C. CORE INVARIANTS",
        ...
        (
            "I. KNOWN LIMITATIONS",
            f"The lexical baseline is intentionally shallow. Provider quality is not guaranteed, and deterministic validation cannot prove unrestricted natural-language entailment. The post-refactor live-provider evaluation is non-deterministic even though its surrounding contracts are deterministic; it recorded {facts.live_evaluation_provider_timeouts} corpus timeouts, {facts.live_fixture_provider_failures} fixture provider failures, semantic readiness `{facts.live_evaluation_readiness}`, and availability `{facts.live_evaluation_availability}`. Observed semantic failures still block baseline review, and observed availability failures independently block hosted-readiness confidence. No True North evaluation baseline has been accepted. Deployment and hosted acceptance have not occurred. Historical and True North artifact families remain intentionally incomparable.",
        ),
        (
            "J. INTERACTION MODEL",
