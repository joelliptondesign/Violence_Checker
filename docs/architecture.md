...
`src/evaluation/reporting.py` renders the machine regression artifact into ten stable engineering sections covering provenance, corpus, coverage, outcomes, patterns, validation, policy, representative cases, and evidence-supported opportunities. It reads repository artifacts only and uses no provider or LLM. `src/evaluation/artifact_cli.py` exposes explicit `accept-baseline`, `compare-run`, and `generate-report` commands. No full live-provider corpus run, accepted baseline artifact, generated report artifact, or semantic performance claim is committed. Evaluation results cannot automatically modify the semantic pipeline.
