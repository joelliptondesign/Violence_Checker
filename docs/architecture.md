...
| `AcceptedBaselineArtifact` | `src/evaluation/regression_contracts.py` | explicit acceptance in `src/evaluation/baseline.py` | deterministic regression comparison |
| `RegressionArtifact` | `src/evaluation/regression_contracts.py` | `src/evaluation/regression.py` | engineering reporting and machine-readable inspection |
...
`src/evaluation/corpus.py` reads only the canonical JSON corpus, rejects duplicate JSON keys, validates strict corpus and evaluation schema version `1.0.0`, rejects unknown fields and malformed or non-synthetic cases atomically, verifies identifier uniqueness and ordering, validates bounded category and tag coverage, and confirms ground truth against existing validation, compatibility, and policy authorities. Generated runs, baselines, and reports are outside corpus authority and cannot affect corpus loading. Its deterministic coverage summary reports category, documentation tag, success/failure, current/historical, policy-outcome, and compound-case counts without executing semantic extraction or assigning a benchmark score.
...
`src/evaluation/case_comparison.py` compares only explicitly asserted expectations. It excludes provider confidence from semantic equality, preserves stable field paths and ordering, distinguishes validation or pipeline failure from semantic mismatch, and treats provider or infrastructure failure as non-comparable. Compatibility failure is emitted only for explicit compatibility-construction failure provenance; an admissible constructed finding that differs from expected compatibility evidence is a compatibility difference. Evidence coverage uses deterministic exact containment across ordered observed spans, so longer exact excerpts and exact segmentation do not create omissions, while excerpts absent from the supplied normalized narrative remain unsupported. Event-type disagreement is separate from uncertainty-note disagreement. Legacy excessive and insufficient uncertainty values remain in the bounded vocabulary only so prior artifacts can be loaded.

`src/evaluation/run_contracts.py` defines immutable configuration, observation, summary, and artifact envelopes. Canonical JSON artifacts are written under `evaluation/runs/` as observed evidence, never truth; an existing path is protected unless overwrite is explicit. The deterministic test suite executes the complete 48-case corpus without network access.

`src/evaluation/baseline.py` accepts a complete run only through explicit invocation and snapshots its ordered observations, case evaluations, summary, execution identity, and acceptance provenance into a strict immutable artifact under `evaluation/baselines/`. Baseline files are never overwritten. Explicit replacement requires a new identifier and artifact path that records the retained prior baseline identifier.

`src/evaluation/regression.py` compares compatible ordered case sets against ground truth using the accepted and current case evaluations. A prior mismatch becoming a match is improved; a prior match becoming a comparable mismatch is degraded; provider or infrastructure failure is incomparable; and identical outcome state is unchanged. A mismatch changing into another mismatch remains unchanged at the outcome level while introduced and resolved field differences and failure patterns remain visible. Deterministic aggregates cover categories, failure-pattern deltas, policy outcomes, validation stages, and provider failures.
...
- independent evaluation contracts, corpus loading, validation, coverage, complete-pipeline execution, baseline acceptance, regression comparison, engineering reporting, and canonical serialization in `evaluation/`
